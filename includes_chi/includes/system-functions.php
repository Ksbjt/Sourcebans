...
/*************************************************************************
	...
	Copyright ÂŠ 2014-2016 SourceBans++ Dev Team <https://github.com/sbpp>
	...
	This program is based off work covered by the following copyright(s):
		SourceBans 1.4.11
		Copyright ÂŠ 2007-2014 SourceBans Team - Part of GameConnect
...
{
    global $userbank;
    AddTab("主页", "index.php?p=home", "This page shows an overview of your bans and servers.");
    AddTab("服务器", "index.php?p=servers", "All of your servers and their status can be viewed here");
    AddTab("封禁", "index.php?p=banlist", "All of the bans in the database can be viewed from here.");
    if ($GLOBALS['config']['config.enablecomms'] == "1") {
        AddTab("聊天封禁", "index.php?p=commslist", "All of the communication bans (such as chat gags and voice mutes) in the database can be viewed from here.");
    }
    if ($GLOBALS['config']['config.enablesubmit']=="1") {
        AddTab("举报", "index.php?p=submit", "You can submit a demo or screenshot of a suspected cheater here. It will then be up for review by one of the admins");
    }
    if ($GLOBALS['config']['config.enableprotest']=="1") {
        AddTab("申诉", "index.php?p=protest", "Here you can appeal your ban. And prove your case as to why you should be unbanned.");
    }
    if ($userbank->is_admin()) {
        AddTab(" 管理员面板 ", "index.php?p=admin", "This is the control panel for SourceBans where you can setup new admins, add new server, etc.");
    }
    ...
    if ($userbank->HasAccess(ADMIN_OWNER|ADMIN_LIST_ADMINS|ADMIN_ADD_ADMINS|ADMIN_EDIT_ADMINS|ADMIN_DELETE_ADMINS)) {
        $submenu->addMenuItem("管理员", 0, "", "index.php?p=admin&amp;c=admins", true);
    }
    if ($userbank->HasAccess(ADMIN_OWNER|ADMIN_LIST_SERVERS|ADMIN_ADD_SERVER|ADMIN_EDIT_SERVERS|ADMIN_DELETE_SERVERS)) {
        $submenu->addMenuItem("服务器", 0, "", "index.php?p=admin&amp;c=servers", true);
    }
    if ($userbank->HasAccess(ADMIN_OWNER|ADMIN_ADD_BAN|ADMIN_EDIT_OWN_BANS|ADMIN_EDIT_GROUP_BANS|ADMIN_EDIT_ALL_BANS|ADMIN_BAN_PROTESTS|ADMIN_BAN_SUBMISSIONS)) {
        $submenu->addMenuItem("封禁", 0, "", "index.php?p=admin&amp;c=bans", true);
    }
    if ($userbank->HasAccess(ADMIN_OWNER|ADMIN_ADD_BAN|ADMIN_EDIT_OWN_BANS|ADMIN_EDIT_ALL_BANS)) {
        $submenu->addMenuItem("聊天封禁", 0, "", "index.php?p=admin&amp;c=comms", true);
    }
    if ($userbank->HasAccess(ADMIN_OWNER|ADMIN_LIST_GROUPS|ADMIN_ADD_GROUP|ADMIN_EDIT_GROUPS|ADMIN_DELETE_GROUPS)) {
        $submenu->addMenuItem("组", 0, "", "index.php?p=admin&amp;c=groups", true);
    }
    if ($userbank->HasAccess(ADMIN_OWNER|ADMIN_WEB_SETTINGS)) {
        $submenu->addMenuItem("设置", 0, "", "index.php?p=admin&amp;c=settings", true);
    }
...
function BuildBreadcrumbs()
{
    ...
    if (!empty($bread)) {
        $text = "&raquo; <a href='index.php?p=home'>主页</a> &raquo; " . $bread;
    } else {
        $text = "&raquo; <a href='index.php?p=home'>主页</a>";
    }
